...
/* eslint-disable react-hooks/refs, react-hooks/set-state-in-effect --
   Vendored MapCN/MapLibre wrapper (installed via the shadcn registry, not
   authored in this project). It uses the standard "latest ref" pattern to
   give imperative MapLibre event handlers access to current props/state
   without resubscribing, which the React Compiler lint rules flag even
   though it predates and is unrelated to compiler usage here. */
...
if (typeof window !== 'undefined' && !MapLibreGL.getWorkerUrl()) {
  MapLibreGL.setWorkerUrl('/map-assets/maplibre-gl-worker.mjs');
}
...
function MapControls({
  ...
  const handleZoomIn = useCallback(() => {
    map?.zoomTo(map.getZoom() + 1, { duration: reduced ? 0 : 300 });
  }, [map, reduced]);
  ...
  const handleResetBearing = useCallback(() => {
    map?.resetNorthPitch({ duration: reduced ? 0 : 300 });
  }, [map, reduced]);
  ...
  }, [map, onLocate, onLocateError, reduced]);
